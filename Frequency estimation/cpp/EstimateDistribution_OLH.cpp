...
#define RES_FILE "../results/Res_OLH"
...
// Add noise to events
void AddNoise(int *ev, int ev_num, int cat_num, double eps, int g, int *support_num) {
	int ev_now;
	int hindex, oindex, oindex2;
	double rnd;
	double p;
	int i, j;
	...
	for (i = 0; i < ev_num; i++) {
		...
		if (eps != -1.0) {
			// generate a hash value for ev_now --> hindex
			hindex = genrand_int32() % g;

			// (exp(eps) - 1) / (exp(eps) + g - 1) --> p
			p = (exp(eps) - 1) / (exp(eps) + (double)g - 1.0);

			// generate random value --> rnd
			rnd = genrand_real1();

			// output hindex with prob p --> oindex
			if(rnd < p) oindex = hindex;
			// output a hash value generated from a uniform distribution --> oindex
			else{
				oindex = genrand_int32() % g;
			}
			...
			for (j = 0; j < cat_num; j++){
				// ev_now
				if (j == ev_now){
					// add 1 for ev_now if hindex == oindex
					if (hindex == oindex) support_num[j] += 1;
				}
				// other input categories
				else{
					// generate a hash value --> oindex2
					oindex2 = genrand_int32() % g;
					if (hindex == oindex2) support_num[j] += 1;
				}
			}
		}
		else{
			support_num[ev_now] += 1;
		}
	}
...
void EmpiricalEstimation(double *est_dist, int cat_num, int *support_num, double eps, int g, int decoder, int ev_num){
	double est_dist_sum;
	double a, b;
	int i, j;
	...
	// absolute frequency
	// g * (exp(eps) + g - 1) / ((exp(eps) - 1)(g - 1)) --> a
	if (eps != -1.0) a = (double)g * (exp(eps) + (double)g - 1.0) / ((exp(eps) - 1.0) * ((double)g - 1.0));
	else a = 1.0;
	// ev_num * (exp(eps) + g - 1) / ((exp(eps) - 1)(g - 1)) --> b
	if (eps != -1.0) b = (double)ev_num * (exp(eps) + (double)g - 1.0) / ((exp(eps) - 1.0) * ((double)g - 1.0));
	else b = 0.0;
	/*
	// relative frequency
	// g * (exp(eps) + g - 1) / ((exp(eps) - 1)(g - 1)) --> a
	...
	// (exp(eps) + g - 1) / ((exp(eps) - 1)(g - 1)) --> b
	if (eps != -1.0) b = (exp(eps) + (double)g - 1.0) / ((exp(eps) - 1.0) * ((double)g - 1.0));
	else b = 0.0;
	*/
	...
	if (decoder == 1){
		...
		// absolute frequency
		// var = (exp(eps) + g - 1)^2 * ev_num / ((exp(eps) - 1)^2 * (g - 1))
		if (eps != -1.0) sd = (exp(eps) + (double)g - 1.0) * sqrt((double)ev_num) / ((exp(eps) - 1.0) * sqrt((double)g - 1.0));
		// relative frequency
		// var = (exp(eps) + g - 1)^2 / ev_num / ((exp(eps) - 1)^2 * (g - 1))
//		if (eps != -1.0) sd = (exp(eps) + (double)g - 1.0) / sqrt((double)ev_num) / ((exp(eps) - 1.0) * sqrt((double)g - 1.0));
		else sd = 0.0;
...
//	init_genrand(1);
	...
	if (argc < 4) {
		...
		printf("[OBF_MECHANISM]: Obfuscation mechanism (1: OLH)\n");
...
//		cout << epsl<< " " << G << endl;
		...
		for (rn = 0; rn < RUN_NUM; rn++){
			...
			AddNoise(Event, UserNum, CatNum, epsl, G, SupportNum);
//			for(i=0;i<CatNum;i++) cout << i << " " << EventDist[i] << " " << SupportNum[i] << endl;
			...
			for (em = 0; em < est_method_num; em++) {
				...
				EmpiricalEstimation(EstEventDist, CatNum, SupportNum, epsl, G, est_method[em], UserNum);
